...
async function submitCheckin() {
  ...
  try {
    const newRow = {
      ...
      engine_hours: parseFloat(document.getElementById('ci-engine-hours').value) || null,
      starter_v:    parseFloat(document.getElementById('ci-batt-starter-v').value) || null,
      starter_pct:  parseInt(document.getElementById('ci-batt-starter-pct').value) || null,
      aux_v:        parseFloat(document.getElementById('ci-batt-aux-v').value) || null,
      aux_pct:      parseInt(document.getElementById('ci-batt-aux-pct').value) || null,
      fuel_l:       parseFloat(document.getElementById('ci-fuel-l').value) || null,
    };
...
async function saveTurretChecklist() {
  ...
  if (!vehicleId) { showToast("⚠ Please select a vehicle first"); return; }
  ...
  const data = {
    ...
    lowlink_splate: chk('chk-lowlink_splate'),
    lowlink_echute: chk('chk-lowlink_echute'),
    uppflex_chute:  chk('chk-uppflex_chute'),
    lowflex_chute:  chk('chk-lowflex_chute'),
    lws_comp:       chk('chk-lws_comp'),
    scu:            num('chk-scu'),
    dcu:            num('chk-dcu'),
    fault_list:     txt('chk-fault_list'),
    notes:          txt('chk-notes'),
  };
  ...
  hl.innerHTML = records.map(r => `
    <div class="history-item">
      ...
      <div class="history-grid">
        ...
        <div class="history-field"><span>Starter:</span> ${safe(r.starter_v,'—')}V · ${safe(r.starter_pct,'—')}%</div>
        <div class="history-field"><span>Aux:</span> ${safe(r.aux_v,'—')}V · ${safe(r.aux_pct,'—')}%</div>
